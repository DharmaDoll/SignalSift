...
運用通知に失敗しても他情報源の記事処理と成功済み状態保存を妨げないが、実行結果は終了コード1とする。情報源障害だけで運用通知の送信に成功した場合は終了コード0とし、定期実行全体を失敗扱いにしない。`--dry-run` では送信せず `operational-alert-preview` として表示する。同じ障害が次の定期実行でも続く場合は、復旧を見逃さないことを優先し、実行ごとに通知する。
