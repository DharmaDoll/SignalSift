...
Workflowログの`fetch=failed`と運用通知を確認します。1情報源の障害は他の情報源の処理を止めず、成功した記事の通知・state更新も継続します。通常実行では運用通知が成功すれば、その情報源障害だけでWorkflow全体は失敗扱いになりません。運用通知自体に失敗した場合は終了コード1になるため、Actionsのエラーとして対応してください。Feedの構造変更が疑われる場合は、まずdry-runで候補数とエラー内容を確認してください。
